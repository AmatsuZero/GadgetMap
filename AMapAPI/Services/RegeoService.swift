...
public struct RegeoRequest {
    public enum RegeoRoadMapLevel: Int {
        /// 显示所有道路
        case all = 0
        /// 过滤非主干道路，仅输出主干道路数据
        case main = 1
    }
    public enum RegeoPOIStrategy: Int {
        /// 不对召回的排序策略进行干扰。
        case none = 0
    ...
    /// 道路等级
    public var roadlevel: RegeoRoadMapLevel?
    public var sig: Bool
    /// 返回数据格式类型
    ...
    /// 是否优化POI返回顺序
    public var strategy: RegeoPOIStrategy?
    public init(location: [CLLocation], poitype: [String]? = nil,
...
public struct RegeoResponse: Codable {
    ...
    public struct RegeoCode: Codable {
